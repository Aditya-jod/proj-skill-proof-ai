...
# --- SRP: IntegrityEventParser ---
class IntegrityEventParser:
    @staticmethod
    def parse(event: Dict[str, Any], state: SessionState) -> Dict[str, Any]:
        payload = event.get("payload", {})
        timestamp = datetime.utcnow()
        elapsed = (timestamp - state.integrity.last_event_at).total_seconds()
        pending_inactivity = elapsed > 180
        return {
            "payload": payload,
            "timestamp": timestamp,
            "elapsed": elapsed,
            "pending_inactivity": pending_inactivity,
        }

# --- SRP: IntegrityDecisionMaker ---
class IntegrityDecisionMaker:
    @staticmethod
    def decide(payload: Dict[str, Any], elapsed: float, pending_inactivity: bool) -> AgentDecision:
        event_type = payload.get("event", "unknown")
        metadata = {
            "event": event_type,
            "elapsed_since_last": round(elapsed, 2),
            "inactivity_check": pending_inactivity,
        }
        ...
        if event_type == "webcam_alert":
            metadata["flagged"] = payload.get("flagged", True)
            return AgentDecision(
                agent="integrity",
...
# --- SRP: IntegrityStateUpdater ---
class IntegrityStateUpdater:
    @staticmethod
    def update(decision: AgentDecision, state: SessionState, timestamp: datetime, pending_inactivity: bool) -> Dict[str, Any]:
        inactivity_alert = None
        if pending_inactivity:
            inactivity_alert = state.integrity.register_inactivity()
        state.integrity.advance(timestamp)
...
# --- SRP: IntegrityAgent orchestrates the process ---
class IntegrityAgent(BaseAgent):
    ...
    def observe(self, event: Dict[str, Any], state: SessionState) -> None:
        parsed = IntegrityEventParser.parse(event, state)
        self._payload = parsed["payload"]
        self._timestamp = parsed["timestamp"]
        self._elapsed = parsed["elapsed"]
        self._pending_inactivity = parsed["pending_inactivity"]
        self._last_message = ""

    def decide(self, state: SessionState) -> AgentDecision:
        return IntegrityDecisionMaker.decide(self._payload, self._elapsed, self._pending_inactivity)

    def act(self, decision: AgentDecision, state: SessionState) -> Dict[str, Any]:
        result = IntegrityStateUpdater.update(decision, state, self._timestamp or datetime.utcnow(), self._pending_inactivity)
